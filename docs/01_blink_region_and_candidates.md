...
*   **`tutorial/02b_extract_blink_from_matlab_blinker.py`**:
    A key resource for users migrating from MATLAB. It shows how to invoke the Python port of the legacy BLINKER algorithm to replicate the exact detection logic of the original toolbox while resolving the MATLAB runner from the sibling validation workspace instead of a vendored `src/` copy.
